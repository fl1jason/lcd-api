...
Message.findLatestByUserId = (userId, result) => {
  sql.query(`SELECT message_text FROM messages INNER JOIN users ON messages.message_to = users.ID WHERE users.user_name='${userId}' ORDER BY id DESC LIMIT 1`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
